...
CHECK_PROMPT_specification = """You are a JSON format calibrator. Please extract the data from INPUT following the output template below. Please exclude the key "justification" and its value.
...
async def json_parser_specification(key, output, evaluator):
    ...
    if vulnerability_reward == {}:
        ...
        new_output = await evaluator.generate(f"INPUT\n{output['answer']}", system_prompt=CHECK_PROMPT_specification)
